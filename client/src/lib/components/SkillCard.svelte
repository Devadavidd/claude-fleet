...
>
  <div class="flex items-center gap-2">
    ...
    {#if skill.maturity}
      <span class="text-[9px] font-mono text-fleet-warn border border-[#4a3f18] rounded px-1.5 flex-none">{skill.maturity}</span>
    {/if}
  </div>
  ...
  <div class="flex flex-wrap items-center gap-2 mt-auto pt-1">
    ...
    {#if skill.provenance && skill.provenance !== 'upstream'}
      ...
      <span class="text-[9px] font-mono text-fleet-accent border border-[#33346a] rounded px-1.5" data-testid="skill-provenance">{skill.provenance}</span>
    {/if}
